...
def process_ERA5(year, month, time_interval, spatial_resolution, output_folder):
    data = xr.open_dataset(os.path.join(output_folder, f'download_{year}_{month}.nc'))
    ...
    res = comb.coarsen(
        longitude = coarsen_weight, # 5 into 1
        latitude = coarsen_weight,
        boundary='trim'
    ).mean()

    ## Saving
    res.to_netcdf(os.path.join(output_folder, f'download_{year}_{month}_processed.nc'))
